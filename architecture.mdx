...
<Tabs>
  <Tab title="Core" icon="server">
    ...
    |--------|------|-------------|
    | **Auth** | `auth/` | JWT login, setup wizard, refresh, password management |
    | **Users** | `users/` | User CRUD with CE limits (3 users) |
    | **Organizations** | `organizations/` | Organization settings and configuration |
    | **Database** | `database/` | Prisma client, connection management |
    | **Redis** | `redis/` | Cache service with get/set/del/pattern operations |
