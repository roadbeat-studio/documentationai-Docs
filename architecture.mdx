...
graph TB
    ...
    API -->|Consumer BFF| CD
    API -->|Admin proxy| FGW
    FGW -->|Deliver activities| Fediverse[Mastodon / Lemmy / Pleroma]
```
...
<Tabs>
  ...
  <Tab title="Publishing" icon="send">
    ...
    | **Federation** | `federation/` | Admin proxy to Federation Gateway — publisher lifecycle, followers, replies, reactions, moderation, policy |
  </Tab>
...
erDiagram
    ...
    Organization ||--o| FedActor : federates
    FedActor ||--o{ FedFollower : has
    FedActor ||--o{ FedInboundReply : receives
    FedActor ||--o{ FedInboundReaction : receives
    FedActor ||--o{ FedModerationEvent : receives
```
