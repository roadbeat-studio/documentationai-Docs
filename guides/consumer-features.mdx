...
- **View modes** — 10 modes organized in grouped toggle: Activity Feed (social-media timeline with big images), Compact List, Large List (detailed rows with larger thumbnails), Compact Masonry (4-column variable-height), Large Masonry (3-column variable-height), Compact Tiles (4 per row), Large Tiles (3 per row with more info), Calendar, Map (interactive Leaflet/OpenStreetMap with marker clustering), 3D Globe (interactive Mapbox globe with custom markers)
- **Infinite scrolling** — optional auto-loading of next pages on scroll, toggled via pagination controls or facet sidebar
...
The **Masonry** view modes display content in a variable-height multi-column layout where cards preserve their natural image proportions, creating an organic, visually rich grid.

Two variants are available via the grouped toggle:

| Variant | Max Columns | Responsive Breakpoints |
|---------|-------------|----------------------|
| **Compact Masonry** | 4 columns | 1 → 2 → 3 → 4 (at 400/600/800px container width) |
| **Large Masonry** | 3 columns | 1 → 2 → 3 (at 400/600px container width) |

- **Natural image heights** — unlike the fixed-aspect-ratio tile views, masonry cards preserve original image proportions for a more varied, editorial feel
- **JS-based column distribution** — items are assigned to the shortest column (by item count) rather than using CSS `columns`, which would rebalance all items on every DOM change
- **Stable infinite scroll** — when new pages load, only the new items are distributed to columns; existing items never change position. This is achieved by tracking the distribution count and only processing items beyond the previous watermark
- **Responsive columns** — a `ResizeObserver` on the container element dynamically adjusts the column count based on the content area width (not viewport width, since the facet sidebar reduces available space)
- **Append detection** — the component distinguishes between a fresh search (reset all columns) and an infinite scroll append (distribute only new items) by comparing the first item's ID and array length

Each masonry card includes: hero image at natural aspect ratio, content type badge with icon, external link indicator, publisher name, publication date, title (2-line clamp), description (3-line clamp), and bookmark/info action buttons.

### Infinite Scrolling

Infinite scrolling automatically loads the next page of results when the user scrolls near the bottom of the content area, replacing traditional pagination.

#### Enabling & Disabling

- **Enable** — click the ∞ button next to the "Next" button in the pagination controls. The current page's results become the starting accumulation, and subsequent pages load automatically.
- **Disable** — toggle the "Infinite scrolling" switch in the facet sidebar (bottom of the filter panel, only visible when active). This resets to page 1 with standard pagination.

#### How It Works

<Steps>
  <Step title="IntersectionObserver sentinel" icon="eye">
    A hidden sentinel element sits below the last content item. When it enters the viewport (with a 200px margin), the next page loads.
  </Step>
  <Step title="Accumulated teasers" icon="layers">
    Results from each page are appended to an accumulated array. The `displayTeasers` computed signal returns this array during infinite scroll, or the single-page results during normal pagination.
  </Step>
  <Step title="Loading guard" icon="shield">
    The `isLoadingMore` flag prevents concurrent page loads. The sentinel only triggers when the previous load has completed and more pages are available.
  </Step>
</Steps>

#### Keyboard Navigation

Arrow key navigation (↑/↓) works across all accumulated items during infinite scroll. The handler uses the full `displayTeasers` array to find the current selection and move to the adjacent item, regardless of which page it came from. `scrollSelectedIntoView` ensures the newly selected item is visible.

<Callout kind="info">
  Infinite scrolling works with all view modes: feed, list, list-large, masonry, masonry-large, cards, and cards-large. For masonry views, the JS-based column distribution ensures new items append smoothly without reflowing existing columns.
</Callout>
...
Save content and organize into collections. The bookmarks page displays saved items as a **3-column card grid** (default) or a **compact list**, with collection tabs, sorting, and pagination. Clicking any bookmark opens a detail view using the same Content Reader as the Discover page.
...
Clicking a bookmark card or list row opens the full content in the Content Reader, using the same responsive display pattern as the Discover page:

| Mode | Screen Width | Behavior |
|------|-------------|----------|
| **Split pane** | ≥2560px (4K / ultrawide) | Side-by-side layout: left 50% bookmarks list, right 50% content reader. A placeholder is shown when no bookmark is selected. |
| **Modal overlay** | &lt;2560px (regular desktop) | Full-screen modal with focus trap, Escape to close, backdrop dismiss. The left navigation sidebar remains visible. |

The currently selected bookmark is highlighted with a brand-colored ring in both grid and list views. The URL updates to `/consumer/bookmarks/read/:contentType/:id` while reading and restores to `/consumer/bookmarks` when the reader is closed.

### Keyboard Navigation

When the content reader is open in **split pane mode** (4K screens), arrow keys navigate between bookmarks:

| Key | Action |
|-----|--------|
| **↓ ArrowDown** | Open the next bookmark in the list |
| **↑ ArrowUp** | Open the previous bookmark in the list |
| **Escape** | Close the content reader |

The selected bookmark card or list row automatically scrolls into view when navigating with arrow keys. Keyboard navigation is only active in split pane mode — the modal handles its own keyboard events (Escape to close, focus trap).

<Callout kind="info">
  Keyboard navigation uses the same pattern as the Discover page. Each bookmark element carries a `data-bookmark-id` attribute used by `scrollSelectedIntoView` to ensure the active item is visible in the scrollable list.
</Callout>

### View Modes

The bookmarks page supports two view modes, toggled via icon buttons next to the sort dropdown:

- **Card grid** (default) — 3-column responsive grid with thumbnail images, content type badges, titles, descriptions, publisher names, and relative timestamps. Hover reveals collection and delete action buttons.
- **Compact list** — single-column rows with small thumbnails, inline metadata, collection tags, and action buttons.
...
### Bookmark Popover (Discover Integration)

When saving a bookmark from the Discover page, a **popover** appears allowing the user to select one or more collections before saving:

<Steps>
  <Step title="Bookmark created on server" icon="bookmark">
    The bookmark is first created via `POST /bookmarks` and the server confirms the record exists before proceeding.
  </Step>
  <Step title="Collections assigned" icon="folder-plus">
    Selected collections are assigned via `POST /bookmarks/collections/:id/items` — these calls only execute after the bookmark is confirmed on the server.
  </Step>
  <Step title="Last-used collection remembered" icon="clock">
    The first selected collection is stored as "last used" and pre-selected the next time the popover opens for a new bookmark.
  </Step>
</Steps>

<Callout kind="tip">
  The bookmark-then-assign workflow is sequential — the system awaits the bookmark creation response before adding collection memberships. This ensures the backend's `addToCollection` endpoint can find the bookmark record.
</Callout>
...
|------|---------|----------|
| **Split pane** | 4K / ultrawide (≥2560px) | Side-by-side: left 50% content list (Discover or Bookmarks), right 50% reader pane |
